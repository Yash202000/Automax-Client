...
export const CreateRequestModal: React.FC<CreateRequestModalProps> = ({
  ...
  const getPriorityValue = useCallback((): number | undefined => {
    ...
    if (!priorityCategory || !lookupValues[priorityCategory.id])
      return undefined;
  ...
  const saveMutation = useMutation({
    mutationFn: async ({
      ...
      if (response.data && files.length > 0) {
        await Promise.all(
          files.map((file) =>
            requestApi.uploadAttachment(response.data!.id, file),
          ),
        );
      }
      return response;
  ...
  const validate = (): boolean => {
    ...
    if (!classificationId) {
      newErrors.classification = t("requests.fieldRequired", {
        field: t("requests.classification"),
      });
    }
    if (!locationId) {
    ...
    if (priorityCategory) {
      ...
      if (!priorityValue) {
        newErrors["lookup:PRIORITY"] = t("requests.fieldRequired", {
    ...
    for (const field of workflowRequiredFields) {
      if (field.startsWith("lookup:")) {
        ...
        if (category && !lookupValues[category.id]) {
          newErrors[field] = t("requests.fieldRequired", {
            field: category.name,
          });
        }
      }
    ...
    <div className="fixed inset-0 bg-[hsl(var(--foreground)/0.6)] backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[hsl(var(--card))] rounded-xl shadow-2xl max-w-3xl w-full animate-scale-in max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* Additional Details — other workflow-required lookup fields */}
          {requestLookupCategories.some((cat) => cat.code !== "PRIORITY") && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {requestLookupCategories
                .filter((cat) => cat.code !== "PRIORITY")
                .map((category) => {
