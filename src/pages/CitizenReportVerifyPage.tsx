...
import { CitizenReportApi } from "@/api/admin";
...
function resolveError(
  ...
  switch (status) {
    case 410:
      return t(
        "citizenReport.error410",
        "This link has expired. Please contact support.",
      );
    case 401:
      return t(
        "citizenReport.error401",
        "Invalid link. Please use the link from your SMS/email.",
      );
    case 404:
      return t(
...
export function CitizenReportVerifyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (last6digits.length < 6) {
      setError(t("citizenReport.enterAllDigits", "Please enter all 6 digits."));
      return;
    }
    if (!id || !signedToken) {
      setError(
    ...
    mutation.mutate({ id, signed_token: signedToken, last6digits });
  };
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-900">
            {t("citizenReport.title", "Incident Report")}
          </h1>
        </div>
        <iframe
          src={pdfUrl}
          title={t("citizenReport.title", "Incident Report")}
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 justify-start">
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            {t("citizenReport.verifyTitle", "Verify your identity")}
          </h1>
        </div>
        <p className="mt-2 text-sm text-gray-600">
          {t(
            "citizenReport.verifySubtitle",
            "Enter the last 6 digits of the mobile number registered on this incident to view the report.",
          )}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-3">
            {t(
              "citizenReport.mobileDigitsLabel",
          ...
          >
            {digits.map((d, i) => (
              <input
                ...
                `}
                aria-label={`${t("citizenReport.digit", "Digit")} ${i + 1}`}
              />
            ))}
          </div>
        ...
        >
          {t("citizenReport.verifyAndView", "Verify & View Report")}
        </Button>
      </form>

      <p className="mt-6 text-center text-xs text-gray-400">
        {t(
          "citizenReport.secureNote",
          "This link is unique to your incident and will expire.",
        )}
      </p>
    </div>
  );
