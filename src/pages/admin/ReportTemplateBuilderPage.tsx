...
const ELEMENT_TOOLS = [
  ...
  {
    type: "dynamic_field" as ElementType,
    label: "Dynamic Field",
    icon: Calendar,
  },
...
const ReportTemplateBuilderPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "elements" | "page" | "header" | "footer" | "properties"
  >("elements");
  const [isDragging, setIsDragging] = useState(false);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });
  const [selectedHeaderElementId, setSelectedHeaderElementId] = useState<
    string | null
  >(null);
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent) => {
    ...
    const scale =
      canvasRef.current.clientWidth / (pageSettings.size === "A4" ? 210 : 216);
  ...
  const updateHeaderElementContent = (
    elementId: string,
    content: TemplateElement["content"],
  ) => {
    setHeader((prev) => ({
  ...
  const availableFields =
    DATA_SOURCES.find((ds) => ds.key === dataSource)?.fields || [];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <input
              ...
              placeholder={t("reports.saveDialog.templateName")}
            />
            <input
              type="text"
              dir="rtl"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded"
                    />
                    <span className="text-xs">{t("reports.enabled")}</span>
                  </label>
                </div>
                {header.enabled && (
                  <>
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setHeader((prev) => ({
                            ...prev,
                            show_on_all_pages: e.target.checked,
                          }))
                        }
                        className="rounded"
                      />
                      <span className="text-xs">
                        {t("reports.showOnAllPages")}
                      </span>
                    </label>
                    <div className="space-y-2">
                      <button
                        onClick={() => {
                          ...
                          (titleElement.content as TextContent).text =
                            templateName;
                          (titleElement.content as TextContent).font.size = 16;
                          (titleElement.content as TextContent).font.weight =
                            "bold";
                          setHeader((prev) => ({
                            ...prev,
                      ...
                      >
                        <Plus className="h-4 w-4" />
                        {t("reports.addLogoImage")}
                      </button>
                    </div>
                    {header.elements.length > 0 && (
                      <div className="border-t pt-4">
                        <h4 className="text-xs font-medium text-gray-600 mb-2">
                          {t("reports.headerElementsList")}
                        </h4>
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setFooter((prev) => ({
                            ...prev,
                            show_page_number: e.target.checked,
                          }))
                        }
                        className="rounded"
                      />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFooter((prev) => ({
                              ...prev,
                              page_number_format: e.target.value,
                            }))
                          }
                          className="w-full border rounded px-2 py-1.5 text-sm"
                          placeholder={t("reports.pagePageOfTotal")}
                        />
                      </div>
                    )}
                  </>
            ...
            {activeTab === "properties" && (
              <>
                ...
                {selectedHeaderElementId && (
                  <ElementPropertiesPanel
                    element={
                      header.elements.find(
                        (el) => el.id === selectedHeaderElementId,
                      )!
                    }
                    onUpdate={(updates) =>
                      updateHeaderElement(selectedHeaderElementId, updates)
                    }
                    onUpdateContent={(content) =>
                      updateHeaderElementContent(
                        selectedHeaderElementId,
                        content,
                      )
                    }
                    onDelete={() =>
                      deleteHeaderElement(selectedHeaderElementId)
                    }
                    onDuplicate={() => {}}
                    availableFields={availableFields}
                  />
              ...
              >
                ...
                {header.elements.map((el) => (
                  <div
                    ...
                    style={{
                      left: el.position.relative
                        ? undefined
                        : `${el.position.x}mm`,
                      top: el.position.relative
                        ? undefined
                        : `${el.position.y}mm`,
                      width: el.size.width > 0 ? `${el.size.width}mm` : "auto",
                      height: el.size.auto_height
                        ? "auto"
                        : `${el.size.height}mm`,
                      zIndex: el.z_index,
            ...
            >
              {elements.map((element) => (
                <div
                  ...
                  style={{
                    left: element.position.relative
                      ? undefined
                      : `${element.position.x}mm`,
                    top: element.position.relative
                      ? undefined
                      : `${element.position.y}mm`,
                    width:
                      element.size.width > 0
                        ? `${element.size.width}mm`
                        : "auto",
                    height: element.size.auto_height
                      ? "auto"
                      : `${element.size.height}mm`,
                ...
                >
                  {element.type === "text" && (
                    <div
                      style={{
                        fontFamily: (element.content as TextContent).font
                          .family,
                        fontSize: `${(element.content as TextContent).font.size}pt`,
                        fontWeight: (element.content as TextContent).font
                          .weight,
                  ...
                  {element.type === "table" && (
                    <div className="border rounded overflow-hidden">
                      <div className="bg-blue-500 text-white px-2 py-1 text-xs font-medium">
                        {t("reports.dataTable")}
                        {(element.content as TableContent).data_source}
                      </div>
                  ...
                  {element.type === "dynamic_field" && (
                    <div
                      ...
                      style={{
                        fontFamily: (element.content as DynamicFieldContent)
                          .font.family,
                        fontSize: `${(element.content as DynamicFieldContent).font.size}pt`,
                        color: (element.content as DynamicFieldContent).font
                          .color,
                        textAlign: (element.content as DynamicFieldContent)
                          .alignment,
                      }}
                    >
              ...
              {elements.length === 0 && (
                <div className="flex flex-col items-center justify-center h-64 text-gray-400">
                  ...
                  <p className="text-sm">
                    {t("reports.addElementsFromTheToolbox")}
                  </p>
                  <p className="text-xs mt-1">
                    {t("reports.startWithATableToDisplayYour")}
                  </p>
                </div>
              )}
            </div>
            ...
            {footer.enabled && (
              ...
              >
                <div className="flex items-center justify-between h-full text-xs text-gray-400">
                  <span>{t("reports.footerArea")}</span>
                  {footer.show_page_number && (
                    <span>
        ...
        <div className="space-y-2">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                onUpdate({
                  style: { ...element.style, background_color: e.target.value },
                })
              }
              className="w-full h-8 border rounded"
              disabled={element.locked}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  onUpdate({
                    style: { ...element.style, border_color: e.target.value },
                  })
                }
                className="w-full h-8 border rounded"
                disabled={element.locked}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              onUpdate({
                ...content,
                font: { ...content.font, size: Number(e.target.value) },
              })
            }
            className="w-full border rounded px-2 py-1.5 text-sm"
            disabled={disabled}
        ...
        <div>
          ...
          <select
            ...
            disabled={disabled}
          >
            <option value="normal">{t("workflows.typeNormal")}</option>
            <option value="bold">{t("reports.bold")}</option>
          </select>
        </div>
      </div>
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            onUpdate({
              ...content,
              font: { ...content.font, color: e.target.value },
            })
          }
          className="w-full h-8 border rounded"
          disabled={disabled}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onUpdate({
              ...content,
              alignment: e.target.value as TextContent["alignment"],
            })
          }
          className="w-full border rounded px-2 py-1.5 text-sm"
          disabled={disabled}
        >
          <option value="left">{t("reports.left")}</option>
          <option value="center">{t("reports.center")}</option>
          <option value="right">{t("reports.right")}</option>
          <option value="justify">{t("reports.justify")}</option>
        </select>
      </div>
    </div>
  );
    ...
    <div className="space-y-3">
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onUpdate({
              ...content,
              source_type: e.target.value as ImageContent["source_type"],
            })
          }
          className="w-full border rounded px-2 py-1.5 text-sm mb-2"
          disabled={disabled}
        >
          <option value="url">{t("reports.url")}</option>
          <option value="base64">{t("incidents.uploadFile")}</option>
        </select>
        {content.source_type === "url" ? (
          <input
            ...
            className="w-full border rounded px-2 py-1.5 text-sm"
            placeholder={t("reports.httpsExampleComImagePng")}
            disabled={disabled}
          />
        ) : (
          <input
      ...
      <div>
        ...
        <select
          value={content.fit}
          onChange={(e) =>
            onUpdate({ ...content, fit: e.target.value as ImageContent["fit"] })
          }
          className="w-full border rounded px-2 py-1.5 text-sm"
          disabled={disabled}
        >
          <option value="contain">{t("reports.contain")}</option>
          <option value="cover">{t("reports.cover")}</option>
          <option value="fill">{t("reports.fill")}</option>
          <option value="none">{t("reports.columnSelector.none")}</option>
        </select>
      </div>
    </div>
  );
...
const TableElementProperties: React.FC<{
  ...
  const updateColumn = (
    index: number,
    updates: Partial<TableContent["columns"][0]>,
  ) => {
    const newColumns = [...content.columns];
  ...
  return (
    <div className="space-y-3">
      ...
      {content.columns.length > 0 && (
        <div>
          <label className="block text-xs text-gray-500 mb-1">
            {t("reports.columns")} ({content.columns.length})
          </label>
      ...
      <div className="space-y-2">
        <label className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              onUpdate({ ...content, show_header: e.target.checked })
            }
            className="rounded"
            disabled={disabled}
          />
          <span className="text-xs">{t("reports.showHeaderRow")}</span>
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            ...
            onChange={(e) =>
              onUpdate({ ...content, alternate_rows: e.target.checked })
            }
            className="rounded"
            disabled={disabled}
          />
          <span className="text-xs">{t("reports.alternateRowColors")}</span>
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            ...
            onChange={(e) =>
              onUpdate({ ...content, show_row_numbers: e.target.checked })
            }
            className="rounded"
            disabled={disabled}
          />
          <span className="text-xs">{t("reports.showRowNumbers")}</span>
        </label>
      </div>

      <div>
        <label className="block text-xs text-gray-500 mb-1">
          {t("reports.maxRows0All")}
        </label>
        <input
          type="number"
          value={content.max_rows || 0}
          onChange={(e) =>
            onUpdate({ ...content, max_rows: Number(e.target.value) })
          }
          className="w-full border rounded px-2 py-1.5 text-sm"
          disabled={disabled}
          min={0}
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onUpdate({
              ...content,
              field: e.target.value as DynamicFieldContent["field"],
            })
          }
          className="w-full border rounded px-2 py-1.5 text-sm"
          disabled={disabled}
        >
          <option value="date">{t("reports.currentDate")}</option>
          <option value="datetime">{t("reports.currentDateTime")}</option>
          <option value="page_number">{t("reports.pageNumber")}</option>
          <option value="total_pages">{t("reports.totalPages")}</option>
          <option value="custom">{t("reports.customText")}</option>
        </select>
      </div>
      {content.field === "date" || content.field === "datetime" ? (
        <div>
          <label className="block text-xs text-gray-500 mb-1">
            {t("reports.dateFormat")}
          </label>
          <input
            type="text"
            ...
            className="w-full border rounded px-2 py-1.5 text-sm"
            placeholder={t("reports.20060102OrDdMmYyyy")}
            disabled={disabled}
          />
        </div>
      ) : null}
      {content.field === "custom" && (
        <div>
          <label className="block text-xs text-gray-500 mb-1">
            {t("reports.customValue")}
          </label>
          <input
            type="text"
            value={content.custom_value || ""}
            onChange={(e) =>
              onUpdate({ ...content, custom_value: e.target.value })
            }
            className="w-full border rounded px-2 py-1.5 text-sm"
            disabled={disabled}
          />
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onUpdate({
              ...content,
              alignment: e.target.value as DynamicFieldContent["alignment"],
            })
          }
          className="w-full border rounded px-2 py-1.5 text-sm"
          disabled={disabled}
