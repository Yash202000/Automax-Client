...
// "Child Objective" detail page — Process is the model name, but every
// user-facing label calls it Child Objective, the child of a Parent
// Objective (OperationalObjective). Mirrors OperationalObjectiveDetailPage.
export const ProcessDetailPage: React.FC = () => {
  ...
  const lastUpdated = [process?.created_at, ...kpis.map((k) => k.updated_at)]
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-slate-400 dark:text-slate-500 mt-1">
                {process.operational_objective && (
                  <Link
                    to={`/goals/kpi/master-data/operational-objectives/${process.operational_objective.id}`}
                    className="hover:text-blue-600 dark:hover:text-blue-400 hover:underline"
                  >
                    Parent Objective: {process.operational_objective.name_en}
                  </Link>
                )}
                {process.pillar && (
                  <span>
                    {t("kpi.masterData.pillar")}: {process.pillar.name_en}
                  </span>
                )}
                {process.enabler && (
                  <span>
                    {t("kpi.masterData.enabler")}: {process.enabler.name_en}
                  </span>
                )}
                {process.goal && (
                  <span>
                    {t("kpi.masterData.strategicGoal")}: {process.goal.title}
                  </span>
                )}
                {process.department && (
                  <span>
                    {t("kpi.masterData.department")}: {process.department.name}
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="border-b border-slate-200 dark:border-slate-700/60">
        <nav
          ...
          aria-label="Child Objective tabs"
        >
          {tabs.map((tab) => (
            <button
              key={tab.key}
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/80 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-900 dark:text-white">
                  {process.operational_objective?.name_en ?? "-"}
                </p>
              </div>
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">
