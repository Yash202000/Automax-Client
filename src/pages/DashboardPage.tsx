...
export const DashboardPage: React.FC = () => {
  ...
  const incidentsHref =
    isSuperAdmin || hasPermission(PERMISSIONS.INCIDENTS_VIEW)
      ...
      : hasPermission(PERMISSIONS.INCIDENTS_TRANSITION)
        ? "/incidents/my-assigned"
        : "/incidents/my-created";

  const requestsHref =
    isSuperAdmin || hasPermission(PERMISSIONS.REQUESTS_VIEW)
      ? "/requests"
      : hasPermission(PERMISSIONS.REQUESTS_TRANSITION)
        ? "/requests/my-assigned"
        : "/requests/my-created";

  const complaintsHref =
    isSuperAdmin || hasPermission(PERMISSIONS.COMPLAINTS_VIEW)
      ? "/complaints"
      : "/complaints/my-created";
  ...
  const navigationCards: NavigationCard[] = [
    ...
    },
    {
      ...
      description: t(
        "dashboard.manageServiceRequests",
        "Handle and track service requests",
      ),
      href: requestsHref,
      icon: FileText,
